...
export default async function Page({ params }: { params: Promise<{ slug: string }> }) { const { slug } = await params;
  ...
  return (
    <>
      ...
      <Section id="zapchasti"><div className="card p-6 sm:p-8 grid md:grid-cols-[1fr_auto] gap-6 items-center"><div><h2 className="text-2xl">Запчасти {b.name} под заказ</h2><p className="mt-2 text-muted max-w-2xl">Своего склада нет — подбираем по VIN и заказываем с удалённого склада, ожидание от 30 минут. Оригинал и проверенные аналоги{b.group === "jp" ? " (555, KYB, Sankei, Akebono и другие)" : ""}. Ставим сразу, гарантия на работу и деталь. Свои запчасти — та же цена работы.</p></div><Link href="/zapchasti/" className="btn-ghost shrink-0">Как заказать</Link></div></Section>
