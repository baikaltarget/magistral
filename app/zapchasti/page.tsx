...
import { Breadcrumbs, Section, Cta, FaqList } from "@/components/Ui";
...
export default function Page() {
  ...
  return (<>
    ...
    <Section><h2 className="mb-4">Запчасти по маркам</h2><div className="flex flex-wrap gap-2">{BRANDS.filter(b => b.priority === 1).map((b) => <Link key={b.slug} href={`/marki/${b.slug}/`} className="rounded-pill bg-[#F2F3F5] px-4 py-2 font-semibold hover:bg-accent-soft hover:text-accent-deep">Запчасти {b.name}</Link>)}</div></Section>
    ...
    <Section><Cta title="Проще написать в WhatsApp" text="Пришлите VIN и фото детали — подберём и назовём цену." /></Section>
