...
export default function BrandTile({ b }: { b: Brand }) {
  ...
  return (
    <Link href={`/marki/${b.slug}/`} className="card px-4 py-4 hover:border-accent transition flex items-center gap-3">
      {logo && <span className="w-14 h-10 shrink-0 flex items-center justify-center"><Image src={logo} alt={b.name} width={56} height={40} className="max-h-10 max-w-14 w-auto h-auto object-contain grayscale opacity-70" /></span>}
