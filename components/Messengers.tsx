...
export function MaxLink({ className = "w-10 h-10", label = true }: { className?: string; label?: boolean }) {
  return (<a href={SITE.max} target="_blank" rel="noopener" aria-label="Написать в Max" title="Написать в Max" className={`${className} rounded-full overflow-hidden flex items-center justify-center shrink-0`}><Image src="/img/max.png" alt="Max" width={40} height={40} className="w-full h-full" /></a>);
}
